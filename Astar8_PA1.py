...
def main():
    total_steps_count = 0
    # writes to euclidean_distance_astar.txt
    with open('manhattan_distance_astar.txt','w') as out_file:
        out_file.write("A-star\n")
        out_file.write("Manhattan distance\n\n")
        for i in range(5):
            p = EightPuzzle()
            p.randomshuffle(15)
            ...
            for row in range(3):
                ...
                out_file.write("\n")
            out_file.write("-------------\n")
            path, count = p.stepcount_to_solve(manhattan_distance)
            path.reverse()
            for mat in path:
                for row in range(3):
                    for col in range(3):
