        ...
        while len(openList) > 0:
            ...
            for move in succsr:
                ...
                if idm_closed == -1 and idm_open == -1:
                    ...
                    if fscore < copy.hscore + copy.gscore:
                        
                        copy.hscore = hval
                        copy._parent = move._parent
                        copy.gscore = move.gscore
                elif idm_closed > -1:
            ...
            closedList.append(m)
            openList = sorted(openList, key=lambda p: p.hscore + p.gscore)
...
def main():
    total_steps_count = 0
    # writes to euclidean_distance_astar15.txt
    with open('euclidean_distance_astar15.txt','w') as out_file:
        out_file.write("A-star15\n")
        out_file.write("Euclidean distance\n\n")
        for i in range(5):
            p = EightPuzzle()
            p.randomshuffle(15)
            ...
            for row in range(4):
                ...
                out_file.write("\n")
            out_file.write("-------------\n")
            path, count = p.stepcount_to_solve(euclidean_distance)
            path.reverse()
            for mat in path:
                for row in range(4):
                    for col in range(4):
